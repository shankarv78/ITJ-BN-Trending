...
def handle_base_entry(signal: dict) -> dict:
    ...
    if existing and existing.get('status') in ['open', 'partial']:
        logger.warning(f"Position {position_id} already exists (status: {existing.get('status')}), skipping entry")
    ...
    # CRITICAL: Always persist position state when orders are placed, even if CE is pending
    # This prevents unmanaged risk exposure if PE fills but CE is delayed
    if execution_result['status'] in ['success', 'ce_pending', 'ce_status_unknown']:
        ...
        if execution_result['status'] == 'success':
            ...
            logger.warning(f"⚠️  Position {position_id} PARTIAL: PE filled, CE {execution_result['status']} - position tracked for safety")
    else:
...
def handle_pyramid(signal: dict) -> dict:
    ...
    if not base_position or base_position.get('status') not in ['open', 'partial']:
    ...
    existing = state.get_position(position_id)
    if existing and existing.get('status') in ['open', 'partial']:
        logger.warning(f"Pyramid {position_id} already exists (status: {existing.get('status')}), skipping")
    ...
    if execution_result['status'] in ['success', 'ce_pending', 'ce_status_unknown']:
        ...
        if execution_result['status'] == 'success':
            ...
            logger.warning(f"⚠️  Pyramid {position_id} PARTIAL: PE filled, CE {execution_result['status']} - position tracked for safety")
    else:
...
def handle_exit(signal: dict) -> dict:
    ...
    if position.get('status') not in ['open', 'partial']:
        logger.warning(f"Position {position_id} not open/partial, status: {position.get('status')}")

...
Implements the three pillars of position sizing:
1. Risk Control (Lot-R)
2. Volatility Control (Lot-V)
3. Margin Control (Lot-M)
...
class TomBassoPositionSizer:
    ...
    def calculate_base_entry_size(
        ...
        # Formula: (Equity × Vol%) / (ATR × Point_Value)
        ...
        if vol_per_lot <= 0:
            ...
            return TomBassoConstraints(lot_r, 0, 0, 0, "invalid_volatility")
        
        lot_v = vol_budget / vol_per_lot
        
        logger.debug(f"Lot-V: VolBudget={vol_budget:.0f}, ATR={atr:.2f}, "
        ...
        # FINAL: Minimum of all three, floored
        final_lots = math.floor(min(lot_r, lot_v, lot_m))
        final_lots = max(0, final_lots)  # Ensure non-negative
        
        # Determine limiting factor
        min_constraint = min(lot_r, lot_v, lot_m)
        if min_constraint == lot_r:
            limiter = "risk"
        elif min_constraint == lot_v:
            limiter = "volatility"
        else:
            limiter = "margin"

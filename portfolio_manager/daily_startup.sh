...
PM_PORT=5002
OPENALGO_PORT=5000
# EQUITY is now loaded from database automatically (capital transactions)
# Do NOT pass --capital flag to preserve database equity value
...
if curl -s http://127.0.0.1:$PM_PORT/health | grep -q "healthy" 2>/dev/null; then
    ...
    # NOTE: Do NOT pass --capital flag - equity is loaded from database
    # (capital transactions track deposits/withdrawals properly)

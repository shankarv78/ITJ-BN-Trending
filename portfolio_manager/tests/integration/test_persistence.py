...
class TestSignalToDatabaseFlow:
    ...
    def test_base_entry_signal_persisted(self, engine, db_manager):
        ...
        if result['status'] == 'executed':
            ...
            assert position.lots > 0  # Actual lots may differ from suggested
    ...
    def test_pyramid_signal_persisted(self, engine, db_manager):
        ...
        base_result = engine.process_signal(base_signal)
        ...
        # Pyramid may fail if base wasn't executed, or due to divergence/validation
        assert result['status'] in ['executed', 'blocked', 'rejected'], \
            f"Unexpected status: {result['status']}"

        # Only verify persistence if both base and pyramid were executed
        if base_result.get('status') == 'executed' and result['status'] == 'executed':
    ...
    def test_exit_signal_persisted(self, engine, db_manager):
        ...
        if base_result.get('status') != 'executed':
            ...
            assert result['status'] == 'executed', \
                f"Exit failed unexpectedly: {result}"
...
class TestRecoveryFlow:
    ...
    def test_recovery_allows_continued_trading(self, test_db, mock_openalgo):
        ...
        # This is valid business behavior - the key test is that recovery worked
        assert result['status'] in ['executed', 'rejected', 'blocked'], \
            f"Unexpected pyramid status: {result}"

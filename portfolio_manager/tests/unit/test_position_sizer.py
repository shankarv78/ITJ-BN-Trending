...
class TestTomBassoPositionSizer:
    ...
    def test_volatility_based_lots_calculation(self, bank_nifty_config, base_entry_signal_bn):
        ...
        # Expected calculation:
        # Vol% = 0.2% (initial volatility)
        # Vol budget = 5000000 × 0.002 = 10,000
        # ATR = 350 points
        # Vol per lot = 350 × 35 = 12,250
        # Lot-V = 10000 / 12250 = 0.816 lots
        ...
        assert result.lot_v == pytest.approx(0.816, rel=0.01)
    ...
    def test_final_lots_takes_minimum(self, bank_nifty_config, base_entry_signal_bn):
        ...
        # In this scenario:
        # Lot-R = 1.67, Lot-V = 0.816, Lot-M = 11.11
        # MIN = 0.816 → FLOOR = 0 lots (volatility limited)
        ...
        assert result.final_lots == 0  # Volatility constraint blocks entry
    ...
    def test_peel_off_calculation(self, bank_nifty_config):
        ...
        # Risk peel: target=50k, excess=90k, peel=CEIL(90k/14k) = 7 lots
        #
        # Position vol = 300 × 10 × 35 = Rs 105,000 = 2.1% (exceeds 0.3% ongoing!)
        # Vol peel: target=15k, excess=90k, peel=CEIL(90k/10.5k) = 9 lots
        #
        # Final = MAX(7, 9) = 9 lots (volatility requires more peel-off)
        ...
        assert lots_to_peel == 9  # Volatility limit stricter than risk
...
@pytest.mark.parametrize("equity,expected_lots", [
    (1000000, 0),  # 10L - insufficient
    (5000000, 0),  # 50L - volatility limited to 0
    (10000000, 1),  # 1Cr - can afford 1 lot
])

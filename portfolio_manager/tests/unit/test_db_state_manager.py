...
class TestPyramidingStateOperations:
    ...
    def test_save_pyramiding_state(self, db_manager, sample_position):
        """Test saving pyramiding state"""
        # First save the base position to satisfy FK constraint
        db_manager.save_position(sample_position)
    ...
    def test_get_pyramiding_state(self, db_manager, sample_position):
        """Test loading pyramiding state"""
        # First save the base positions to satisfy FK constraints
        db_manager.save_position(sample_position)

        # Create another position for GOLD_MINI
        gold_position = Position(
            ...
            lots=2,
            quantity=2,
            initial_stop=65000.0,
            current_stop=65500.0,
            highest_close=67000.0,
            status="open",
            is_base_position=True
        )
        db_manager.save_position(gold_position)
...
class TestSignalDeduplication:
    ...
    def test_check_duplicate_signal_exists(self, db_manager):
        ...
        # Log a signal - use ISO format for datetime (JSON serializable)
        signal_data = {
            'instrument': 'BANK_NIFTY',
            'type': 'BASE_ENTRY',
            'position': 'Long_1',

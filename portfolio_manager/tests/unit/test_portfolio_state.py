...
class TestPortfolioStateManager:
    ...
    def test_portfolio_risk_calculation_multiple_positions(
        ...
        # BN risk: (52000 - 51800) × 5 lots × 30 = 200 × 5 × 30 = Rs 30,000
        # Total: Rs 45,000 = 0.9% of 50L

        gold_risk = 500 * 3 * 10   # Rs 15,000
        bn_risk = 200 * 5 * 30     # Rs 30,000
        total_risk = gold_risk + bn_risk  # Rs 45,000

        assert state.total_risk_amount == pytest.approx(total_risk)
        assert state.gold_risk_percent == pytest.approx(0.3, rel=0.01)
        assert state.banknifty_risk_percent == pytest.approx(0.6, rel=0.01)
        assert state.total_risk_percent == pytest.approx(0.9, rel=0.01)

...
Handles:
- Bank Nifty monthly expiry (last Wednesday of month)
- Gold Mini expiry (last calendar day of month)
...
def get_banknifty_expiry(target_date: datetime = None) -> Tuple[datetime, str]:
    ...
    expiry_date = get_last_wednesday_of_month(year, month)
...
def get_gold_mini_expiry(target_date: datetime = None) -> Tuple[datetime, str]:
    """
    Get Gold Mini futures expiry date (last day of month)
...
def format_expiry_string(expiry_date: datetime) -> str:
    """
    Format expiry date as YYMONDD string

    Args:
        expiry_date: datetime object

    Returns:
        String like "25DEC25" or "25DEC31"
    """

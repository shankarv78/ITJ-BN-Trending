...
class CrashRecoveryManager:
    ...
    def __init__(self, db_manager):
        ...
        self.consistency_epsilon = 0.01  # 1 paisa tolerance for financial calculations
            ...
            if position.status == "open":
                try:
                    ...
                    if hasattr(trading_engine, 'openalgo_client') and trading_engine.openalgo_client:
                        ...
                        point_value = 35.0 if position.instrument == "BANK_NIFTY" else 10.0
    ...
    def _validate_state_consistency(
        ...
        """
        Validate that reconstructed positions mathematically align with portfolio totals
        
        Checks:
        1. Sum of position risks == portfolio_state.total_risk_amount
        2. Sum of position margins == portfolio_state.margin_used (if available)
        ...
        # Calculate current state from positions
        current_state = portfolio_manager.get_current_state()
        
        # Validation 1: Risk amount consistency
        if 'total_risk_amount' in portfolio_state and portfolio_state['total_risk_amount'] is not None:
            db_risk = float(portfolio_state['total_risk_amount'])
            calculated_risk = current_state.total_risk_amount

            risk_diff = abs(db_risk - calculated_risk)
            if risk_diff > self.consistency_epsilon:
                error_msg = (
                    f"Risk amount mismatch: DB={db_risk:,.2f}, "
                    f"Calculated={calculated_risk:,.2f}, Diff=₹{risk_diff:,.2f}"
                )
                logger.error(error_msg)
                return False, error_msg
        
        # Validation 2: Margin consistency (if available)
        if 'margin_used' in portfolio_state and portfolio_state['margin_used'] is not None:
            db_margin = float(portfolio_state['margin_used'])
            calculated_margin = current_state.margin_used

            margin_diff = abs(db_margin - calculated_margin)
            if margin_diff > self.consistency_epsilon:
                error_msg = (
                    f"Margin amount mismatch: DB={db_margin:,.2f}, "
                    f"Calculated={calculated_margin:,.2f}, Diff=₹{margin_diff:,.2f}"
                )
                logger.error(error_msg)
                return False, error_msg
